...
class DynamicRouter:
    def __init__(self):
        ...
        self.pub_topics = ["/sh_rh_wrj1_position_controller/command", "/sh_rh_wrj2_position_controller/command", "/fake_topic",
                           "/sh_rh_ffj0_position_controller/command", "/sh_rh_ffj3_position_controller/command", "/sh_rh_ffj4_position_controller/command", 
                           "/fake_topic", "/sh_rh_mfj0_position_controller/command", "/sh_rh_mfj3_position_controller/command", "/sh_rh_mfj4_position_controller/command", 
                           "/fake_topic", "/sh_rh_rfj0_position_controller/command", "/sh_rh_rfj3_position_controller/command",
                           "/sh_rh_rfj4_position_controller/command", "/fake_topic", "/sh_rh_lfj0_position_controller/command", 
    ...
    def callback(self, msg):
        ...
        for i in range(len(data)):
            limited_value = self.clamp(data[i], self.joints_limits[i][0], self.joints_limits[i][1])
            self.publishers[i].publish(Float64(limited_value))
            self.joint_states[i] = limited_value
